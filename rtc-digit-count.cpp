...
namespace rtc_digit_count {
	...
	template<uint64_t count = 100000ull>
		requires(count > 0)
	consteval uint64_t max_iterations_for() noexcept {
...
#else
	static constexpr double convergence_threshold{ 1.0 };
	static constexpr double rse_threshold{ 2.5 };
#endif
	...
	static constexpr benchmarksuite::stage_config_data config{ .clear_cpu_caches_before_iterations = true,
		...
		.max_iteration_count																	   = max_iterations_for<100000ull>(),
	...
	template<benchmarksuite::string_literal stage_name_new, benchmarksuite::string_literal type_name, typename v_type> void test_function() {
		...
		std::string header{ make_section00() + benchmarksuite::get_time() + ")\n" + make_section01() };
