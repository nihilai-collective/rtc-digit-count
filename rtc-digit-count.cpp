...
#if defined(NDEBUG)
	static constexpr uint64_t max_iteration_count{ 3000 };
	...
	void test_function(std::string& results_string) {
		...
		for (uint64_t x = 0; x < max_iteration_count; ++x) {
			for (uint64_t y = 0; y < count; ++y) {
				...
				results_02[x][y] = rtc_digit_count(random_integers[x][y]);
			}
		}
		...
		if (results.sorted_results.size() > 0) {
			results_string += generate_section(test_name.operator std::string(), current_path + "/" + stage_name.operator std::string());
			...
			stream << "]";
			bnch_swt::file_handle::save_file(results.to_csv(false), csv_out_path + "/" + stage_name.operator std::string() + "/" + test_name.operator std::string() + ".csv");
			bnch_swt::file_handle::save_file(stream.str(), json_out_path + "/" + stage_name.operator std::string() + "/" + test_name.operator std::string() + ".json");
		}
	}
