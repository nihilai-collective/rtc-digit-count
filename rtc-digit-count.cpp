...
namespace rtc_digit_count {
	...
	template<typename value_type> std::vector<value_type> generate_random_integers(uint64_t count, benchmarksuite::random_generator<value_type>& rg, uint64_t min_digits = 1,
		uint64_t max_digits = max_decimal_digits<value_type>()) {
		std::vector<value_type> randomNumbers(count);
		for (uint64_t x = 0; x < count; ++x) {
			uint64_t digit_count{ rg.impl(min_digits, max_digits) };
...
#if BNCH_SWT_PLATFORM_LINUX && BNCH_SWT_COMPILER_CLANG
		alignas(64) static constexpr uint32_t digitCounts_32[]{ 10, 10, 9, 9, 9, 8, 8, 8, 7, 7, 7, 7, 6, 6, 6, 5, 5, 5, 4, 4, 4, 4, 3, 3, 3, 2, 2, 2, 1, 1, 1, 1, 1 };
		alignas(64) static constexpr uint32_t digitCountThresholds_32_by_clz[]{ 4294967295u, 4294967295u, 999999999u, 999999999u, 999999999u, 99999999u, 99999999u, 99999999u,
			9999999u, 9999999u, 9999999u, 9999999u, 999999u, 999999u, 999999u, 99999u, 99999u, 99999u, 9999u, 9999u, 9999u, 9999u, 999u, 999u, 999u, 99u, 99u, 99u, 9u, 9u, 9u, 9u,
			9u };
		const uint32_t clz{ static_cast<uint32_t>(std::countl_zero(inputValue)) };
		const uint32_t originalDigitCount{ digitCounts_32[clz] };
		return originalDigitCount + static_cast<uint32_t>(inputValue > digitCountThresholds_32_by_clz[clz]);
#else
		alignas(64) static constexpr uint32_t digitCounts_32[]{ 10, 10, 9, 9, 9, 8, 8, 8, 7, 7, 7, 7, 6, 6, 6, 5, 5, 5, 4, 4, 4, 4, 3, 3, 3, 2, 2, 2, 1, 1, 1, 1, 1 };
	...
	std::string make_section01(std::string_view cpu_name) {
		std::string result;
		result.reserve(1080);
		...
		result += " -> ...) up to a per-test-size maximum (scaled so every size gets the same iterations*count "
				  "memory/verification budget - smaller integer counts get proportionally more iterations to average out timing noise)";
		result += ". Each epoch runs all iterations and evaluates a trailing window of "
				  "max(iterations/10, ";
		...
		result += "% epoch-over-epoch simultaneously. "
				  ...
				  "If convergence is never reached before ";
		...
		result += "#### Note:\n"
				  "  These benchmarks were executed using the CPU benchmark library "
				  "[benchmarksuite](https://github.com/realtimechris/benchmarksuite).\n  ";
		return result;
	}
	...
	std::string make_section00(std::string_view os_id, std::string_view os_version, std::string_view compiler_id, std::string_view compiler_version) {
		std::string result;
		result.reserve(164);
		result += "# Digit Counting\nPerformance profiling of functions (Compiled and run on ";
		result += os_id;
		result += " ";
		result += os_version;
		result += " using the ";
	...
	std::string generate_section(std::string_view test_name_new, std::string_view current_path_new) {
		...
		result += +"/" + encoded + "-Results.png\" target=\"_blank\"><img src=\"./graphs/";
		result += current_path_new;
		result += "/" + encoded + "-Results.png?raw=true\" \nalt=\"\" width=\"400\"/></p>\n\n";
		return result;
	}
	...
	void run_benchmark_scaled(arg_types&&... args) {
		...
		test_data_val.results[library_name.operator std::string_view()] = benchmarksuite::run_adaptive_benchmark<functor_type, per_test_config>(std::forward<arg_types>(args)...);
	}
	...
	template<typename bench_stage_type, uint64_t count, benchmarksuite::string_literal stage_name, benchmarksuite::string_literal test_name, typename value_type,
		uint64_t min_digits = 1, uint64_t max_digits = max_decimal_digits<value_type>()>
	void test_function(std::string& results_string) {
		...
		std::vector<value_type> random_integers = generate_random_integers<value_type>(total_elements, rg, min_digits, max_digits);
	...
	template<benchmarksuite::string_literal stage_name, benchmarksuite::string_literal type_name, typename v_type, uint64_t count, uint64_t min_digits, uint64_t max_digits>
	void run_digit_range_case(std::string& results_string) {
		using benchmark_type = benchmarksuite::benchmark_stage<stage_name, config>;
		static constexpr benchmarksuite::string_literal count_literal{ benchmarksuite::internal::to_string_literal<count>() };
		static constexpr benchmarksuite::string_literal min_literal{ benchmarksuite::internal::to_string_literal<min_digits>() };
		static constexpr benchmarksuite::string_literal max_literal{ benchmarksuite::internal::to_string_literal<max_digits>() };
		static constexpr auto test_name{ type_name + "-integer_count[" + count_literal + "]-digit_length[" + min_literal + "-" + max_literal + "]" };
		test_function<benchmark_type, count, stage_name, test_name, v_type, min_digits, max_digits>(results_string);
	}

	template<benchmarksuite::string_literal stage_name, benchmarksuite::string_literal type_name, typename v_type, uint64_t min_digits, uint64_t max_digits, uint64_t... counts>
	void run_digit_range_sizes(std::string& results_string, std::integer_sequence<uint64_t, counts...>) {
		(run_digit_range_case<stage_name, type_name, v_type, counts, min_digits, max_digits>(results_string), ...);
	}
	...
	template<benchmarksuite::string_literal stage_name_new, benchmarksuite::string_literal type_name, typename v_type> void test_function() {
		...
		using benchmark_type = benchmarksuite::benchmark_stage<stage_name, config>;
		using counts_seq	 = std::integer_sequence<uint64_t, 10, 100, 1000, 10000, 100000>;
		std::string results_string{};
		test_function<benchmark_type, 10, stage_name, type_name + "-integer_count[10]-digit_length[1-" + digit_length + "]", v_type>(results_string);
		test_function<benchmark_type, 100, stage_name, type_name + "-integer_count[100]-digit_length[1-" + digit_length + "]", v_type>(results_string);
		test_function<benchmark_type, 1000, stage_name, type_name + "-integer_count[1000]-digit_length[1-" + digit_length + "]", v_type>(results_string);
		test_function<benchmark_type, 10000, stage_name, type_name + "-integer_count[10000]-digit_length[1-" + digit_length + "]", v_type>(results_string);
		test_function<benchmark_type, 100000, stage_name, type_name + "-integer_count[100000]-digit_length[1-" + digit_length + "]", v_type>(results_string);

		if constexpr (max_decimal_digits<v_type>() <= 10) {
			run_digit_range_sizes<stage_name, type_name, v_type, 1, 5>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 1, 10>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 5, 10>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 10, 10>(results_string, counts_seq{});
		} else {
			run_digit_range_sizes<stage_name, type_name, v_type, 1, 5>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 1, 10>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 1, 15>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 1, 20>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 5, 10>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 5, 15>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 5, 20>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 10, 15>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 10, 20>(results_string, counts_seq{});
			run_digit_range_sizes<stage_name, type_name, v_type, 15, 20>(results_string, counts_seq{});
		}

		auto results = benchmark_type::get_all_results();
		std::string time_string{ benchmarksuite::get_time() };

		std::string header{ make_section00(static_cast<std::string>(benchmarksuite::system_info_data<benchmarksuite::benchmark_types::cpu>::os_id),
