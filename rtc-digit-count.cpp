...
namespace rtc_digit_count {
	...
	void test_function(std::string& results_string) {
		std::vector<std::vector<value_type>> random_integers{ max_executions };
		std::vector<std::vector<uint64_t>> counts{ max_executions };
		std::vector<std::vector<value_type>> results_01{ max_executions };
		std::vector<std::vector<value_type>> results_02{ max_executions };
		std::vector<std::vector<value_type>> results_03{ max_executions };
		...
		using lemire_subject = bnch_swt::functor_subject<"lemire-digit-count-" + bit_size, lemire_digit_count_type<count>>;
		using fast_subject	 = bnch_swt::functor_subject<"fast-digit-count-" + bit_size, fast_digit_count_type<count>>;
		using rtc_subject	 = bnch_swt::functor_subject<"rtc-digit-count-" + bit_size, rtc_digit_count_type<count>>;
		bench_stage_type::template run_until_resolved<test_name, lemire_subject, fast_subject, rtc_subject>(random_integers, results_02);
		results_string += generate_section(test_name.operator std::string(), current_path + "/" + std::string{ stage_name });
		auto results = bench_stage_type::get_test_results(test_name.operator std::string());
